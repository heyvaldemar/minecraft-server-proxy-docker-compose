...
COMPOSE_FILE="minecraft-server-proxy-docker-compose.yml"
PROJECT="${COMPOSE_PROJECT_NAME:-minecraft-proxy}"
DRY_RUN=false
ALLOW_MAJOR=false
for arg in "$@"; do
  case "$arg" in
...
if [ "$new_major" != "$cur_major" ] && [ "$ALLOW_MAJOR" != "true" ]; then
  ...
  echo "  https://github.com/heyvaldemar/minecraft-server-proxy-docker-compose/releases/tag/$latest" >&2
  exit 3
fi
